...
2. Login to a JASMIN scientific analysis server from the `login` server
   ...
   > [!NOTE]
   > The purpose of having two SSH terminal sessions running on the same `sci` server is to facilitate both compute and monitoring. One terminal is for executing commands on the `sci` server, while the other terminal is for monitoring user processes (or editing a script).
...
5. Make changes to the Python example and re-execute it  
   - Lower the size of the random numbers `nran` from 1024 to 500. Use the following command:\
...
> [!NOTE]
> If you are familiar with one of the text editors `vim`, `nano`, or `emacs`, use one to open the file `random-number-gen.py`, edit the variable `nran` and assign 500 instead of 1024.
...
You will be able to run and test a script on the scientific analysis servers. You will be able to monitor the resources used by your script. You can scale up by using the high-memory scientific `sci-ph-0[1,2].jasmin.ac.uk` servers for testing, and then move your workflow to the batch cluster LOTUS.

#### What tasks I can not run on the `sci` server?

- Do not run processes with an execution time of over two hours
- Do not run parallel applications, e.g., MPI or OpenMP, or high threaded codes on the `sci` servers
- Do not run data transfer processes on the `sci` servers. Please use a transfer `xfer-vm-0[1-3].jasmin.ac.uk` server, except when moving data from `/work/scratch-pw[2,3]` to a GWS (group workspace), as `/work/scratch-pw[2,3]` are not mounted on the `xfer` servers
...
- It is necessary to consider moving a processing task to the batch system LOTUS when the resource demand (e.g., CPU, memory, processing time) is high
...
1. Monitor your processes on the `sci` server by opening another tab side-by-side, and logging into the same `sci` machine
   ...
   ![A screenshot of a terminal running the top command while a Python script is in the Run state](images/Top-output-nran1024-run-new.png)
   ![A screenshot of a terminal running the top command while a Python script is in the Sleep state](images/Top-output-nran1024-sleep.png)
   ...
   - Sort all processes per CPU usage
...
1. Make changes to the Python example and re-execute it
   ...
   ```
    ...
    ![A screenshot of a terminal running the top command while a Python script is in the Sleep state](images/Top-output-nran500-sleep.png)
...
There is no limit on the number of processes launched by a user on the scientific analysis servers. However, the user should limit the number of processes to a maximum of 2 as the `sci` server is shared by other users. Distribute processing tasks across other `sci` servers, and consider moving the tasks to the batch cluster LOTUS.
...
> 5. What text editors or IDE are available on JASMIN?
...
- How many threads did the process spawn?
...
- Edit the script in a text editor, uncomment the line of code `#os.environ[“OMP_NUM_THREADS”] = “2"`, and save the script
...
- On the monitoring terminal, execute the command  `top -H -u <username>` or `ps -T -p <pid>`
   ![A screenshot of a terminal running the top command](images/Top-output-threads.png)
   using the `ps -T -p <pid>`
   ![A screenshot of a terminal running the ps command](images/ps-output-threads.png)

- How many threads did the process spawn?

   7 threads with the master process (e.g., PID 16566 in the example above)
...
- Did the setting `OMP_NUM_THREADS=1` disable multithreading?
   ...
   ![A screenshot of the terminal running the top command, showing a single python process](images/Top-output-threads-set-to-one.png)

- Edit the script in a text editor, uncomment the line of code `#os.environ[“OMP_NUM_THREADS”] = “2"`, and save the script, or use the command `sed`:

   ```bash
   sed -i 's/#os.environ/os.environ/'  dot-product-2arrays.py
   ```

   ![A screenshot of the edited scipt in a text editor](images/Uncomment-line-threads.png)
    Note: This setting must be done before `import numpy` - see screenshot above.
